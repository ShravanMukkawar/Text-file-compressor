...
int huffman::binary_to_decimal(const string& in) // Changed to const reference
{
	int result = 0;
...
inline void huffman::build_tree(const string& path, char a_code) // Changed to const reference
{
...
{
	in_file.open(in_file_name, ios::in); //ios is namepace in cpp use for defined input/output operation in cpp
	in_file.get(id);
	while (!in_file.eof())
	{
		node_array[id]->freq++;
...
void huffman::recreate_huffman_tree()
{
	...
	unsigned char size; // unsigned char to get number of nodes of huffman tr ee
	in_file.read(reinterpret_cast<char*>(&size), 1);
	root = new huffman_node;
	for (int i = 0; i < size; i++)
	{
		char a_code;
		unsigned char h_code_c[16]; // 16 unsigned chars to obtain the binary code
